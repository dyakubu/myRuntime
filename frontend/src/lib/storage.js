...
const Cache = {
  ...
  // An entry is { contract, suite } — the whole generated contract plus its verified
  // suite, so re-opening a problem restores both without re-running generation.
  async get(hash) {
    const db = await openDb();
    return new Promise((resolve, reject) => {
      const tx = db.transaction(STORE, "readonly");
      const req = tx.objectStore(STORE).get(hash);
      // ?? null also makes pre-rewrite records (which stored a differently-shaped
      // field) read as a miss, so stale broken suites regenerate instead of replaying.
  ...
  async set(hash, entry) {
    ...
    return new Promise((resolve, reject) => {
      ...
      tx.objectStore(STORE).put({ hash, entry, savedAt: Date.now() });
