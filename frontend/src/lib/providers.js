...
- Every parameter and return type in function_signature and methods must be one of EXACTLY these, or a nesting of them: int, float, str, bool, None, Any, List[T], Dict[str, T], Optional[T]. Nothing else is accepted — the request is rejected outright otherwise.
  - Dict keys must be str. JSON object keys are always strings, so Dict[int, ...] cannot be delivered: it would arrive with string keys and break any solution that indexes it by int. If a problem is naturally keyed by integers (a graph adjacency map, say), either key it by str or use an index-based List[...] instead.
  - Tuple, Set and your own class names are not usable either — JSON has no equivalent. Use List[...] at the boundary and convert inside reference_solution.code.
  - If the problem is naturally about a node/pointer structure (a tree, a linked list, a graph as an object), represent it as a plain nested list/dict at the boundary (e.g. a level-order array with nulls for a tree) and build/tear down any node objects privately inside reference_solution.code — never require the caller to pass in or receive one of your own classes.
- normalized_statement must describe the input exactly as the solver will actually receive it. If the adjacency map is keyed by str, say so; never promise integer keys, tuples or sets that the parameter types don't deliver.
